...
import { SearchIcon, MenuVerticalIcon, LoadingCircle, PanelRight } from "./Icons";
...
function Header() {
  ...
  return (
    <>
      <header className="w-full z-30 bg-white shadow-sm">
  <div className="max-w-6xl mx-auto px-4 sm:px-6">
    <div className="flex items-center justify-between h-12 md:h-16">
      {/* Site branding */}
      <div className="shrink-0 mr-5">
        {/* Logo container */}
        <div className="shrink-0 mr-8 p-4">
          <Link href="/" className="text-primary">
            <img src="/logo-blue.svg" className="h-8" alt="Logo" />
          </Link>
        </div>
      </div>
      ...
      <div className="md:hidden flex items-center space-x-4">
        ...
        >
          <svg
            className="w-6 h-6"
            fill="none"
            stroke="black" // Changed to black for visibility on white background
            viewBox="0 0 24 24"
          >
            <path
              strokeLinecap="round"
      ...
      {/* Desktop navigation */}
      <nav className="hidden md:flex grow text-primary">
        {/* Desktop menu items */}
        <ul className="flex flex-row items-center space-x-4">
          ...
          <li className="flex items-center">
            <a href="#explore" className="ml-2 text-black hover:text-gray-600 flex items-center">
              <span role="img" aria-label="explore" className="mr-2">🔍</span> Explore
            </a>
          </li>
          <li className="flex items-center">
            <a href="#notifications" className="ml-2 text-black hover:text-gray-600 flex items-center">
              <span role="img" aria-label="notifications" className="mr-2">🔔</span> Notifications
            </a>
          </li>
          <li className="flex items-center relative group">
            <a href="#more" className="ml-2 text-black hover:text-gray-600 flex items-center">
              <span role="img" aria-label="more" className="mr-2">➕</span> More
            </a>
            {/* Submenu for "More" */}
            <ul className="absolute top-full left-0 bg-white shadow-lg hidden group-hover:block">
              <li>
                <a href="#settings" className="block px-4 py-2 text-black hover:bg-gray-100">⚙️ Settings</a>
              </li>
              <li>
                <a href="#help" className="block px-4 py-2 text-black hover:bg-gray-100">❓ Help</a>
              </li>
              <li>
                <a href="#about" className="block px-4 py-2 text-black hover:bg-gray-100">ℹ️ About</a>
              </li>
              <li>
          ...
          {user ? (
            <>
              <li className="flex items-center relative ml-1 mr-1">
                ...
                >
                  <User details={user} height={24} /> {/* Reduced user icon size */}
                  <MenuVerticalIcon />
                </div>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 bg-black bg-opacity-50 z-40">
          <div className="absolute top-12 right-0 bg-white shadow-lg w-64 rounded-lg">
            <ul className="flex flex-col space-y-2 p-4">
              <li>
                <a href="#home" className="flex items-center p-2 text-black hover:bg-gray-100 rounded-lg">
                  <span role="img" aria-label="home" className="mr-2">🏠</span> Home
                </a>
              </li>
              <li>
                <a href="#explore" className="flex items-center p-2 text-black hover:bg-gray-100 rounded-lg">
                  <span role="img" aria-label="explore" className="mr-2">🔍</span> Explore
                </a>
              </li>
              <li>
                <a href="#notifications" className="flex items-center p-2 text-black hover:bg-gray-100 rounded-lg">
                  <span role="img" aria-label="notifications" className="mr-2">🔔</span> Notifications
                </a>
              </li>
              <li className="relative">
                <a href="#more" className="flex items-center p-2 text-black hover:bg-gray-100 rounded-lg">
                  <span role="img" aria-label="more" className="mr-2">➕</span> More
                </a>
                {/* Submenu for "More" on mobile */}
                <ul className="pl-4 mt-2 space-y-2">
                  <li>
                    <a href="#settings" className="flex items-center p-2 text-black hover:bg-gray-100 rounded-lg">
                      <span role="img" aria-label="settings" className="mr-2">⚙️</span> Settings
                    </a>
                  </li>
                  <li>
                    <a href="#help" className="flex items-center p-2 text-black hover:bg-gray-100 rounded-lg">
                      <span role="img" aria-label="help" className="mr-2">❓</span> Help
                    </a>
                  </li>
                  <li>
                    <a href="#about" className="flex items-center p-2 text-black hover:bg-gray-100 rounded-lg">
                      <span role="img" aria-label="about" className="mr-2">ℹ️</span> About
                    </a>
                  </li>
                  <li>
                    <a href="#contact" className="flex items-center p-2 text-black hover:bg-gray-100 rounded-lg">
...
/** Badge showing the new notifications count if any */
  ...
  return (
    <div className="flex flex-row ml-1" onClick={() => setShowNotifPane(true)}>
      <div className="flex flex-row text-brand-hover rounded hover:bg-slate-300/[.2] px-3 py-2 cursor-pointer">
        ...
        {countNewNotifs > 0 && (
          <div className="bg-red-500 rounded-full text-black font-bold py-0.5 px-1.5 text-xs" style={{ marginLeft: 6 }}>
            {countNewNotifs}
          </div>
        )}
      </div>
            ...
            {notifications.length > 0 ? (
              <>
                {notifications.map((notification, key) => {
                  return <NotificationItem notification={notification} key={key} />;
                })}
              </>
            ) : (
                        ...
                        {posts.map((post, key) => {
                          if (post.content.master) {
                            return (
                              ...
                              >
                                · {post.content.title ? post.content.title : <>{post.content.body.substring(0, 25)}...</>}
                              </Link>
                            );
                          } else {
                            return (
              ...
              <div className="flex h-full flex-col overflow-y-scroll bg-white shadow-xl">
                <div className="bg-brand py-6 px-4 sm:px-6">
                  <div className="flex items-center justify-between">
                    <h2 className="text-3xl font-medium text-black deacon" id="slide-over-title">Community Chat</h2>
                    <div className="ml-3 flex h-7 items-center">
                      <button type="button" className="rounded-md bg-transparent text-black hover:text-black focus:outline-none focus:ring-2 focus:ring-white" onClick={() => hide()}>
